...
import {ArrowRight,Bike,CheckCircle2,CreditCard,Eye,EyeOff,FileText,IdCard,Lock,Mail,MapPin,Phone,Search,ShieldCheck,UploadCloud,User} from "lucide-react";
import LocationPicker from "../../components/LocationPicker.jsx";
import { axiosWithoutCreds } from "../../api/axiosInstances.js";
import { registerDeliveryPartnerApi } from "../../api/deliveryPartnerApi.js";
import deliveryPartnerImage from "../../assets/delivery_partner_image.png";
...
function FileField({ label, name, fileName, onFileChange }) {
  return (
...
export const DeliveryPartnerRegisteration = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!address.trim() || !hasSelectedLocation) {
      setLocationError("Please select your current delivery location.");
      setFormError("Please select your current delivery location.");
      return;
    }

    if (!fileNames.aadhaarDocument || !fileNames.licenseDocument) {
      setFormError("Please upload Aadhaar card and driving license documents.");
      return;
    }
    ...
    try {
      ...
      const response = await registerDeliveryPartnerApi(formData);
      setSubmitMessage(
        response?.message ||
          "Delivery partner registration submitted successfully. Your account is pending approval."
      );
    } catch (error) {
      setFormError(error.message || "Unable to register delivery partner.");
    } finally {
      ...
      <div className="h-full w-full bg-white shadow-[0_24px_80px_rgba(225,29,72,0.12)] lg:pl-[380px] xl:pl-[400px]">
        <aside className="fixed inset-y-0 left-0 z-10 hidden w-[380px] overflow-hidden bg-[#fff5f6] lg:block xl:w-[400px]">
          <img
            ...
            alt="BloodConnect delivery partner registration"
          ...
          <div className="mx-auto max-w-none">
            ...
            <form onSubmit={handleSubmit} className="min-h-[calc(100vh-1rem)] rounded-xl rounded-r-none border border-r-0 border-slate-200 bg-white p-3 shadow-sm sm:p-4">
              <div className="mb-3 border-b border-slate-100 pb-3">
                <h1 className="text-xl font-bold tracking-tight text-slate-950">Delivery Partner Registration</h1>
              </div>
              ...
              <section className="space-y-2.5">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <Field label="Full Name" required icon={User} placeholder="Enter full name" inputProps={{ name: "fullName", required: true }} />
                  ...
                  <Field label="Confirm Password" required helper={showConfirmPasswordError ? "Password and confirm password do not match" : ""}>
                    <div className={`flex h-9 items-center gap-2.5 rounded-md border bg-white px-3 shadow-sm transition focus-within:ring-4 ${
                      ...
                      <input
                        ...
                        className="h-full min-w-0 flex-1 bg-transparent text-xs text-slate-900 outline-none placeholder:text-slate-400"
                      />
                      <button type="button" onClick={() => setShowConfirmPassword((current) => !current)} className="grid h-7 w-7 shrink-0 place-items-center rounded text-slate-400 transition hover:text-slate-700" aria-label={showConfirmPassword ? "Hide confirm password" : "Show confirm password"}>
              ...
              <section className="mt-4 space-y-2.5 rounded-lg border border-rose-100 bg-rose-50/70 p-3">
                <h2 className="flex items-center gap-2 text-sm font-bold text-slate-950">
                ...
                <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_220px]">
                  ...
                  <div>
                    <span className="mb-1.5 block text-xs font-semibold text-slate-800">Or Pick on Map</span>
                    <button type="button" onClick={() => setShowLocationPicker(true)} className="flex h-9 w-full cursor-pointer items-center justify-center gap-2 rounded-md border border-slate-200 bg-white px-4 text-xs font-bold text-slate-700 shadow-sm transition hover:border-rose-200 hover:text-rose-700">
                      <MapPin className="h-4 w-4" />
                      {hasSelectedLocation ? "Change Location" : "Pick on Map"}
                    </button>
                    {hasSelectedLocation && <p className="mt-1.5 text-xs font-semibold text-emerald-700">Location selected</p>}
                    {locationError && <p className="mt-1.5 text-xs font-semibold text-rose-600">{locationError}</p>}
                  </div>
                </div>
                <p className="flex items-center gap-2 text-xs font-semibold text-slate-500">
                  <ShieldCheck className="h-3.5 w-3.5 text-slate-400" />
                  Your location helps us assign delivery requests nearby.
                </p>
              </section>
              ...
              <section className="mt-4 space-y-2.5 border-t border-slate-100 pt-3">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <Field label="Aadhaar Number" required icon={CreditCard} placeholder="Enter Aadhaar number" inputProps={{ name: "aadhaarNumber", required: true }} />
                  <FileField label="Aadhaar Card Document" name="aadhaarDocument" fileName={fileNames.aadhaarDocument} onFileChange={handleFileChange("aadhaarDocument")} />
                </div>
              </section>

              <section className="mt-4 space-y-2.5 border-t border-slate-100 pt-3">
                <h2 className="flex items-center gap-2 text-sm font-bold text-slate-950">
                  <Bike className="h-4 w-4 text-rose-600" />
                  Vehicle Information
                </h2>
                <div className="grid gap-3 md:grid-cols-2">
                  <Field label="Vehicle Type" required>
                    <select name="vehicleType" required defaultValue="" className="h-9 w-full rounded-md border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 shadow-sm outline-none transition focus:border-rose-400 focus:ring-4 focus:ring-rose-100">
                      <option value="" disabled>Select vehicle type</option>
                      <option value="bike">Bike</option>
                      <option value="scooter">Scooter</option>
                    </select>
                  </Field>
                  <Field label="Vehicle Number" required icon={Bike} placeholder="Enter vehicle number" inputProps={{ name: "vehicleNumber", required: true }} />
                </div>
              </section>
              ...
              <section className="mt-4 space-y-2.5 border-t border-slate-100 pt-3">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <Field label="Driving License Number" required icon={IdCard} placeholder="Enter license number" inputProps={{ name: "drivingLicenseNumber", required: true }} />
                  <FileField label="License Document" name="licenseDocument" fileName={fileNames.licenseDocument} onFileChange={handleFileChange("licenseDocument")} />
                </div>
              </section>
              ...
              <div className="mt-4 border-t border-slate-100 pt-3">
                {submitMessage && (
                  <div className="mb-3 flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-900">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-bold">Registration checked</p>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <button type="submit" disabled={isSubmitting} className="inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-md bg-rose-600 px-8 text-xs font-bold text-white shadow-lg shadow-rose-100 transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:bg-rose-300">
                    {isSubmitting ? "Registering..." : "Register as Delivery Partner"}
